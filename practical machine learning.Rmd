...
# Model Selection 
Since we prioritise prediction accuracy over interpretability, we consider a random forest paradigm first.
```{r ranfor,cache=TRUE}
...
We try to fit a new model by boosting. 
```{r boost}
...
mod2<-xgboost(data=trainx,label=trainy,maxdepth=2,eta=1,nthread=2,nrounds=30,objective="multi:softmax",num_class=5)
#inbag test
...
```
# Conclusion
Comparing mod1 and mod2, the random forest in mod1 seems to be a better model as it yields highe precision and accuracy on both the training and test data. Class C is most difficult to predict in model 1 
